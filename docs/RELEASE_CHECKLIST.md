...
- [ ] `docs/ROADMAP.md` reflects the actual release stage and remains the single canonical roadmap.
...
- [ ] No Project Zomboid code/assets or decompiled game source have been extracted and redistributed without an explicit rights basis.
...
- [ ] Workshop preview artwork meets current Steam/PZ technical requirements.
...
- [ ] Transient track/mark storage is bounded and cleanup/expiration behavior has been validated.
- [ ] Persistent wear survives save/reload and dedicated-server restart.
- [ ] Existing and late-joining multiplayer clients converge on current persistent state.
...
- [ ] The distributable payload under `Contents/mods/pz-happytrails/` contains only files intended for end users.
- [ ] Repository-only engineering/research documentation is not accidentally copied into the in-game payload.
- [ ] Development logs, private data, secrets, credentials, local paths, and test artifacts are excluded.
- [ ] `workshop-description.bbcode` matches the actual release feature set and status.
